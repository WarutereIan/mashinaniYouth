...
type Category = "Launch" | "Empowerment" | "Education" | "Philanthropy" | "Sports" | "Advocacy";
...
const REGIONS = ["All regions", ...Array.from(new Set(PROJECTS.map((p) => p.region))).sort()];
...
function ProjectsPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {tab === "activities" ? <ActivitiesTab /> : tab === "barazas" ? <BarazasTab /> : <MediaTab />}
...
function BarazasTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return BARAZAS.filter((b) => {
      ...
      if (q) {
        const hay =
          `${b.project} ${b.county} ${b.sector} ${b.venue} ${b.agenda} ${b.attendees}`.toLowerCase();
        if (!hay.includes(q)) return false;
      }
      return true;
    });
    ...
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-soft transition hover:-translate-y-1 hover:shadow-lg">
      {poster ? (
        ...
        <div className="flex items-center justify-between gap-2 px-5 pt-5">
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-widest ${meta.classes}`}
          >
            <Icon className="h-3 w-3" /> {project.category}
          </span>
          <span
            className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest ${STATUS_CLASSES[project.status]}`}
          >
            {project.status}
          </span>
        </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <dl className="mt-4 space-y-2 text-sm">
          ...
          <div className="flex items-start gap-2 text-ink">
            <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-flag-red" />
            <dd>
              {project.venue} <span className="text-muted-foreground">· {project.region}</span>
            </dd>
          </div>
          <div className="flex items-start gap-2 text-muted-foreground">
            <Users className="mt-0.5 h-4 w-4 shrink-0 text-accent" />
