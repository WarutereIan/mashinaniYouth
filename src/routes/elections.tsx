        ...
        <div className="grid gap-5 lg:grid-cols-[1.55fr_1fr]">
          ...
          <div className="rounded-2xl border border-border bg-card p-6">
            <div className="flex items-baseline justify-between">
              <div className="font-display text-lg">All candidates</div>
              <div className="text-[10px] uppercase tracking-widest text-muted-foreground">
                {filteredCandidates.length} shown · {positionCandidates.length} total
              </div>
            </div>
                  ...
                  <div key={c.id} className="rounded-xl border border-border bg-background/40 p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="truncate text-xs text-muted-foreground">
                          {positionTitle.split(" — ")[0]} · {c.county}
                        </div>
                      </div>
                      <div className="font-display text-sm tabular-nums text-primary">{pct}%</div>
                    </div>
                    <div className="mt-1 line-clamp-1 pl-12 text-xs italic text-muted-foreground">
                      "{c.slogan}"
                    </div>
                    <div className="mt-2 flex gap-2">
                      ...
                      <Button
                        size="sm"
                        className="h-7 flex-1 bg-gradient-gold text-xs"
                        onClick={() => handleQuickVote(c.id)}
                        disabled={!!voter && (!eligibleHere || myVote === c.id || !pollsOpen)}
                      >
                        {myVote === c.id ? (
                          ...
                          <>
                            <Vote className="mr-1 h-3 w-3" /> Vote
                          </>
                        )}
                      </Button>
                    </div>

                    <div className="mt-1 hidden text-[10px] text-muted-foreground tabular-nums sm:block">
