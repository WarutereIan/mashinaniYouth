-- M26: Candidate cannot vote in a position they are vying for.
-- Builds on M24's cast_vote (preserves the vote.cast / vote.change audit wiring).
-- A user with an active (pending or approved) candidacy for a position is blocked
-- from casting a ballot in that same position. Rejected candidates may still vote.
-- Linkage: candidates.user_id = auth.uid().
...
-- Privileges preserved by CREATE OR REPLACE; re-grant defensively.
