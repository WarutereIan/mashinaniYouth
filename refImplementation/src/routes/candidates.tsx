...
import { COUNTY_NAMES, constituenciesForCounty, wardsForConstituency } from "@/lib/kenya-locations";
    ...
    <article className="group relative overflow-hidden rounded-2xl border border-border bg-card transition-shadow hover:shadow-lg">
      ...
      <div className="p-5">
        ...
        <div className="mt-2 flex gap-2">
          <Button size="sm" className="flex-1 bg-emerald-600 text-white hover:bg-emerald-700" onClick={onVerify}>
        ...
        {candidate && (
          <>
            ...
            <div className="space-y-3 rounded-xl border border-border bg-card p-4 text-sm">
              ...
              <Row label="Position" value={candidate.position_title} />
              <Row
                label="Scope"
                value={
                  candidate.tier === "county"
              ...
              />
              <Row label="National ID" value={maskId(candidate.national_id)} mono />
              <Row label="IEBC voter no." value={candidate.iebc_voter_number} mono />
              <Row
                label="Certificate no."
                value={candidate.certificate_number ?? "—"}
                mono
                highlight
              />
              <Row
                label="Issued"
                value={
                  candidate.certified_at
                    ? new Date(candidate.certified_at).toLocaleString()
                    : "—"
                }
              />
            </div>
            <div className="flex items-center gap-2 rounded-lg bg-accent/10 px-3 py-2 text-xs text-accent">
              <CheckCircle2 className="h-4 w-4" /> Signed by MY-KDM Elections Directorate ·
              ...
              <Button
                ...
                onClick={() => {
                  if (candidate.certificate_number) {
                    navigator.clipboard.writeText(candidate.certificate_number);
                    setCopied(true);
