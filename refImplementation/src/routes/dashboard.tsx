...
export const Route = createFileRoute("/dashboard")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your MY-KDM voter dashboard: registration details, ballot history and eligible positions.",
      },
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mym:votes-changed", bump);
    window.addEventListener("mym:voter-changed", onVoter);
    return () => {
      window.removeEventListener("mym:votes-changed", bump);
      ...
      <div className="min-h-screen bg-background">
        ...
        <section className="mx-auto max-w-3xl px-4 py-24 text-center sm:px-6">
          ...
          <p className="mx-auto mt-3 max-w-lg text-muted-foreground">
            Your MY-KDM dashboard shows your registration, ballot history and the
            positions you're eligible to vote on. Register in under a minute to
            unlock it.
          </p>
  ...
  const votedIds = positionsVotedByMe(voter.id);
  const eligiblePositions = POSITIONS.filter((p) => isEligible(voter, p));
      ...
      <section className="border-b border-border/60 bg-gradient-to-b from-primary/5 to-background">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
                  <span className="inline-flex items-center gap-1">
                    <MapPin className="h-3.5 w-3.5" /> {voter.ward}, {voter.constituency}, {voter.county}
                  </span>
                  <span className="inline-flex items-center gap-1">
                    <ShieldCheck className="h-3.5 w-3.5 text-sage" /> ID •••{voter.id.slice(-4)}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            <StatCard label="Ballots cast" value={votedPositions.length.toString()} icon={CheckCircle2} tone="sage" />
            <StatCard label="Eligible ballots" value={eligiblePositions.length.toString()} icon={Vote} tone="primary" />
        ...
        <div className="rounded-2xl border border-border bg-card p-6">
          ...
          <div className="mt-5 space-y-3">
            ...
            {votedPositions.map((position) => {
              const myCandidateId = getMyVote(position.id, voter.id);
              const cand = CANDIDATES.find((c) => c.id === myCandidateId);
              const tally = tallyPosition(position.id);
              const total = tally.reduce((s, r) => s + r.votes, 0);
              const rank = cand ? tally.findIndex((r) => r.candidateId === cand.id) + 1 : 0;
              const myVotes = cand ? tally.find((r) => r.candidateId === cand.id)?.votes ?? 0 : 0;
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 truncate text-xs text-muted-foreground">
                      You voted for <span className="font-semibold text-foreground">{cand?.name ?? "—"}</span>
                    </div>
                  </div>
