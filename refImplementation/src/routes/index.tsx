      ...
      <section className="bg-background">
        ...
        <div className="mx-auto max-w-5xl px-4 pb-10 pt-8 text-center sm:px-6 md:pb-14 md:pt-12">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-base leading-relaxed text-muted-foreground md:mt-8 md:text-lg">
            ...
            structured from the ward level to the National Secretariat. We help government
            deliver on its development agenda — and make sure young people are part of that
            delivery, and are economically empowered through it.
          </p>

          <div className="mt-8 flex flex-wrap justify-center gap-3 md:mt-10">
            <Button size="lg" variant="outline" className="border border-ink/20 border-t-primary bg-background text-base text-ink transition hover:border-primary hover:bg-primary hover:text-primary-foreground" asChild>
              <Link to="/register">Register to vie</Link>
            </Button>
            <Button size="lg" className="bg-gradient-gold text-base text-primary-foreground transition hover:shadow-cta" asChild>
              <Link to="/elections">
      ...
      <section className="border-b border-border bg-background">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 md:py-14">
          <div className="grid grid-cols-3 gap-6">
            <LightStat value="47" label="Counties on the ballot" />
            <LightStat value={String(POSITIONS.length)} label="Live positions" />
            <LightStat value={String(CANDIDATES.length)} label="Certified candidates" />
          </div>
      ...
      <section className="bg-ink text-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-20 sm:px-6 md:grid-cols-[1fr_1.4fr]">
          ...
          <ol className="space-y-4">
            {[
              ["01", "Candidate certification", "Aspirants obtain their MY-KDM clearance certificate to be eligible."],
              ["02", "Campaign period", "Certified candidates campaign for national, county and ward positions."],
              ["03", "Electronic voting", "Elections run county by county on M-Taji with a verifiable digital trail."],
              ["04", "Results & inauguration", "The elected leadership is announced and inaugurated in the open."],
            ].map(([n, t, d]) => (
              <li key={n} className="flex gap-4 rounded-xl border border-white/10 bg-white/[0.04] p-5">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border bg-background p-10 md:p-14">
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button size="lg" variant="outline" asChild>
                <a href="https://m-taji-tracker.vercel.app" target="_blank" rel="noopener noreferrer">
...
function ContestantsSection() {
  ...
  const candidates = CANDIDATES.filter((c) =>
    positions.some((p) => p.id === c.positionId),
  ).slice(0, 8);
  const scrollerRef = useRef<HTMLDivElement>(null);
  const [paused, setPaused] = useState(false);
        ...
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-4 font-display text-4xl leading-tight md:text-5xl">
              ...
              <TypewriterLine
                words={[
                  "New face of Youth Leadership in Kenya",
                  "The new home of youth in Kenya",
                ]}
    ...
    <section className="border-t border-border bg-background">
      <div className="mx-auto max-w-7xl px-4 pt-20 sm:px-6 md:pt-24">
        <div className="max-w-2xl">
          ...
          <p className="mt-3 text-sm text-muted-foreground md:text-base">
            Our patrons open doors, sharpen our policy and mentor young leaders — and every one
            of them keeps a public M-Taji profile so their support is transparent and trackable.
          </p>
        </div>
      </div>
...
function PatronCard({ patron }: { patron: Patron }) {
  ...
  return (
    <article className="group/card flex w-[300px] shrink-0 flex-col overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-soft transition hover:-translate-y-1 hover:shadow-lg">
      <div className={`mx-auto h-28 w-28 overflow-hidden rounded-full ring-2 ring-white/20 ${ringClass}`}>
        <img
          src={patron.photo}
      ...
      <div className="mt-4 text-center">
        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest ${chipClass}`}>
          {patron.category}
        </span>
