...
import { getVoter, signOutVoter, type Voter } from "@/lib/voter-store";
...
const externalNav = [
  { href: `${MTAJI_BASE}/shop/mykdm`, label: "Merchandise" },
];
...
export function SiteHeader() {
  ...
  useEffect(() => {
    setVoter(getVoter());
    const on = () => setVoter(getVoter());
    window.addEventListener("storage", on);
    window.addEventListener("mym:voter-changed", on);
    return () => {
      window.removeEventListener("storage", on);
      window.removeEventListener("mym:voter-changed", on);
    };
  }, [loc.pathname]);
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 md:grid-cols-3">
        ...
        <div className="text-sm">
          ...
          <ul className="space-y-2">
            <li><Link to="/elections" className="text-white/80 hover:text-accent">Live elections</Link></li>
            <li><Link to="/auth" className="text-white/80 hover:text-accent">Sign up / Log in</Link></li>
          </ul>
        ...
        <div className="text-sm">
          ...
          <ul className="space-y-2">
            <li><Link to="/about" className="text-white/80 hover:text-accent">About MY-KDM</Link></li>
            <li>
              <SupportButton
                variant="link"
                className="h-auto p-0 text-flag-red hover:text-accent"
              >
                Support MY-KDM
              </SupportButton>
            </li>
            <li><span className="text-white/60">Powered by M-Taji</span></li>
            <li><span className="text-white/60">In partnership with USLA</span></li>
          </ul>
        </div>
      </div>
      <div className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-4 py-4 text-xs text-white/50 sm:px-6">
          © {new Date().getFullYear()} Mashinani Youth Kazi Delivery Movement. Electronic voting powered by M-Taji.
        </div>
      </div>
    </footer>
  );
